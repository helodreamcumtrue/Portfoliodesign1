...
const Navbar = ({ currentView, setView }) => (
  <nav className="fixed top-0 w-full bg-black/95 backdrop-blur-xl z-50 px-4 md:px-6 py-4 flex justify-between items-center border-b border-white/5">
    <div className="text-lg md:text-xl font-black tracking-tighter cursor-pointer uppercase hover:opacity-70 transition-all" onClick={() => setView('home')}>lakshay jain</div>
    <div className="flex gap-4 md:gap-8 text-[10px] md:text-[11px] font-bold tracking-[0.2em] uppercase">
...
const HomeView = ({ setView }) => (
  <div className="min-h-screen flex flex-col md:flex-row bg-black">
    <div className="w-full md:w-1/2 h-[60vh] md:h-screen relative overflow-hidden group">
      <img src="https://images.unsplash.com/photo-1635776062127-d379bfcbb9c8?auto=format&fit=crop&q=80&w=1600" alt="Hero" className="w-full h-full object-cover grayscale brightness-[0.25] group-hover:scale-105 group-hover:grayscale-0 group-hover:brightness-75 transition-all duration-1000 ease-in-out" />
      <div className="absolute inset-0 flex flex-col items-center justify-center p-8 md:p-12 text-center">
        <h1 className="text-5xl sm:text-7xl md:text-9xl font-black text-white italic tracking-tighter mix-blend-overlay uppercase leading-none">Lakshay<br />Jain</h1>
        <p className="mt-6 md:mt-8 text-white/40 text-[10px] md:text-xs font-black tracking-[0.4em] md:tracking-[0.6em] uppercase">Visual Systems // 2026</p>
      </div>
    </div>
    <div className="w-full md:w-1/2 flex items-center justify-center p-8 md:p-24 bg-neutral-900/40">
      <div className="max-w-md">
        ...
        <p className="text-lg md:text-xl text-neutral-400 mb-8 md:mb-10 leading-relaxed font-light italic">"I build digital products that don't just look beautiful, but solve the complex frictions of everyday life."</p>
        <div className="grid grid-cols-1 gap-4 md:gap-6 mb-10 md:mb-12">
          <div className="flex items-center gap-4 text-neutral-500 group">
            <div className="p-2 md:p-3 rounded-full bg-white/5 group-hover:bg-white/10 transition-all"><Smartphone size={18} md:size={20} /></div>
            <p className="text-[10px] md:text-xs font-bold uppercase tracking-widest">UI/UX Strategy</p>
          </div>
          <div className="flex items-center gap-4 text-neutral-500 group">
            <div className="p-2 md:p-3 rounded-full bg-white/5 group-hover:bg-white/10 transition-all"><Layers size={18} md:size={20} /></div>
            <p className="text-[10px] md:text-xs font-bold uppercase tracking-widest">Product Ecosystems</p>
          </div>
        </div>
        <button onClick={() => setView('work')} className="group w-full md:w-auto px-10 md:px-12 py-4 md:py-5 bg-white text-black rounded-full flex items-center justify-center gap-3 font-black text-sm uppercase tracking-widest shadow-2xl transition-all hover:scale-105 active:scale-95">
...
const WorkView = ({ setView, setSelectedProject }) => (
  <div className="min-h-screen pt-24 md:pt-32 pb-24 px-4 md:px-12 max-w-7xl mx-auto">
    <div className="max-w-2xl mb-12 md:mb-24">
      ...
      <h2 className="text-5xl md:text-8xl font-black tracking-tighter mb-6 md:mb-8 italic">The Archive.</h2>
      <p className="text-neutral-500 text-base md:text-lg border-l-2 border-neutral-800 pl-6 md:pl-8 leading-relaxed italic">A selection of work where visual identity meets functional strategy.</p>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12 md:gap-y-16">
      {PROJECTS.map((project) => (
        <div key={project.id} onClick={() => { setSelectedProject(project); setView('project-detail'); }} className="group cursor-pointer">
          <div className="relative aspect-[4/5] overflow-hidden rounded-2xl md:rounded-3xl bg-neutral-900 border border-white/5 shadow-2xl transition-all duration-500 group-hover:border-white/20">
          ...
          <div className="mt-4 md:mt-6 px-1 flex justify-between items-start">
            <div>
              <h3 className="text-lg md:text-xl font-bold tracking-tight group-hover:text-white transition-colors">{project.title}</h3>
              <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-neutral-500 mt-1">{project.category}</p>
            </div>
            <span className="text-[9px] md:text-[10px] font-bold text-neutral-700 bg-white/5 px-2 py-1 rounded">{project.year}</span>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="min-h-screen pt-20 md:pt-12 pb-24 bg-black">
      <div className="max-w-5xl mx-auto px-4 md:px-6">
        <button onClick={() => setView('work')} className="flex items-center gap-2 text-neutral-500 hover:text-white transition-colors mb-8 md:mb-12 uppercase tracking-[0.2em] md:tracking-[0.3em] text-[10px] font-black">
          <ArrowLeft size={14} /> Back to Archive
        </button>

        {/* HERO IMAGE FIRST */}
        <div className="w-full mb-8 md:mb-16 rounded-[1.5rem] md:rounded-[4rem] overflow-hidden shadow-2xl border border-white/5 bg-neutral-900 group">
          <img 
            src={project.thumbnail} 
            alt={project.title} 
            className="w-full h-auto object-cover transform scale-100 group-hover:scale-105 transition-transform duration-[2000ms] ease-out" 
          />
        </div>
        
        {/* CONTENT STARTS AFTER IMAGE */}
        <div className="mb-12 md:mb-24">
          <h1 className="text-4xl sm:text-6xl md:text-9xl font-black tracking-tighter uppercase leading-[0.9] mb-8 md:mb-12 italic">{project.title}</h1>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 md:gap-12">
            <div className="lg:col-span-3">
              <p className="text-xl md:text-3xl text-neutral-300 font-light leading-relaxed italic border-l-4 border-white/10 pl-6 md:pl-8">{project.description}</p>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-4 md:gap-6 border-t lg:border-t-0 border-white/5 pt-6 lg:pt-0">
              <div><p className="text-[9px] md:text-[10px] font-black uppercase text-neutral-600 mb-1 md:mb-2">Service</p><p className="text-xs md:text-sm font-bold">{project.category}</p></div>
              <div><p className="text-[9px] md:text-[10px] font-black uppercase text-neutral-600 mb-1 md:mb-2">Year</p><p className="text-xs md:text-sm font-bold">{project.year}</p></div>
            </div>
          </div>
          
          {project.pdfLink && (
            <a href={project.pdfLink} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center w-full md:w-auto gap-3 px-8 md:px-10 py-4 md:py-5 bg-white text-black rounded-xl md:rounded-2xl hover:bg-neutral-200 transition-all font-black uppercase text-[10px] md:text-xs tracking-widest mt-8 md:mt-12 shadow-xl">
          ...
          {project.story ? (
            project.story.map((item, idx) => (
              <div key={idx} className="max-w-4xl mx-auto">
                {item.type === 'text' ? (
                  <p className="text-lg md:text-2xl text-neutral-400 leading-relaxed font-light italic">{item.content}</p>
                ) : (
                  <div className="w-full bg-neutral-900 rounded-[1.5rem] md:rounded-[3rem] overflow-hidden shadow-2xl border border-white/5 my-4 md:my-0">
        ...
        <div className="mt-24 md:mt-40 pt-16 md:pt-24 border-t border-white/10 text-center">
            <p className="text-neutral-600 uppercase text-[9px] md:text-[10px] font-black tracking-[0.4em] md:tracking-[0.5em] mb-6 md:mb-8 italic">The story continues</p>
            <button onClick={() => setView('work')} className="text-3xl sm:text-4xl md:text-6xl font-black hover:text-neutral-400 uppercase tracking-tighter transition-all italic underline decoration-white/10 underline-offset-8">Next Case Study</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen pt-32 md:pt-48 pb-24 px-4 md:px-6 max-w-5xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 items-start">
        <div className="space-y-8 md:space-y-12">
          <h1 className="text-6xl sm:text-7xl md:text-9xl font-black tracking-tighter uppercase leading-[0.8] italic">New<br />Chapter.</h1>
          <p className="text-lg md:text-xl text-neutral-500 font-light italic leading-relaxed">Available for freelance opportunities and long-term product partnerships.</p>
          <div className="space-y-4">
            <span className="text-[10px] font-black uppercase text-neutral-600 tracking-[0.3em] block">Inquiries</span>
            <a href="mailto:lakshayjain148@gmail.com" className="text-xl sm:text-2xl md:text-3xl font-bold hover:text-neutral-400 transition-colors block border-b border-white/10 pb-4 overflow-hidden text-ellipsis">lakshayjain148@gmail.com</a>
          </div>
        </div>
        <div className="bg-neutral-900/30 p-8 md:p-12 rounded-[2rem] md:rounded-[3.5rem] border border-white/5 shadow-2xl backdrop-blur-xl">
          {submitted ? (
            <div className="py-16 md:py-24 text-center space-y-6 md:space-y-8">
                <CheckCircle size={40} md:size={48} className="mx-auto text-white animate-bounce" /> 
                <h3 className="text-2xl md:text-3xl font-black uppercase italic tracking-tighter">Transmission Successful</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 md:space-y-10">
              <div className="space-y-2">
                  ...
                  <input type="text" required placeholder="Full Name" className="w-full bg-transparent border-b border-white/10 py-3 md:py-4 focus:outline-none focus:border-white text-base md:text-lg transition-all" />
              </div>
              <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-neutral-600 ml-2">Context</label>
                  <textarea rows="4" required placeholder="Project goals and timeline..." className="w-full bg-transparent border-b border-white/10 py-3 md:py-4 focus:outline-none focus:border-white text-base md:text-lg transition-all" />
              </div>
              <button type="submit" className="w-full bg-white text-black font-black uppercase py-5 md:py-6 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center gap-3 text-xs md:text-sm shadow-2xl active:scale-[0.98] transition-all hover:bg-neutral-200 tracking-widest">Send Transmission <Send size={18} /></button>
            </form>
          )}
        </div>
      </div>
...
export default function App() {
  ...
  return (
    <div className="bg-black text-white min-h-screen font-sans selection:bg-white selection:text-black overflow-x-hidden">
      ...
      <footer className="py-12 md:py-20 border-t border-white/5 text-center text-neutral-700 text-[8px] md:text-[10px] uppercase font-black tracking-[0.5em] md:tracking-[1em] px-4">&copy; {new Date().getFullYear()} Lakshay Jain // Product Strategy // Visual Systems</footer>
    </div>
  );
}
