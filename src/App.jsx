...
const CATEGORY_MAP = {
  'juaari-brand':        'UI Design',
  'aavana-brand':        'UI Design',
...
const ALL_CATEGORIES = ['All', 'UI Design', 'Printables', 'Social Media', 'Merch'];

const MERCH_COMING_SOON = true;
...
const WorkView = ({ setView, setSelectedProject }) => {
  ...
  const countFor = (cat) => {
    ...
    if (cat === 'Merch') return 0;
    return PROJECTS.filter(p => CATEGORY_MAP[p.id] === cat).length;
  };
        ...
        <main className="flex-1 min-w-0 pt-10 md:pt-14 pb-24 px-6 md:px-12 mt-12 md:mt-0 md:ml-[192px]">
          ...
          {activeCategory === 'Merch' ? (
            <div className="flex flex-col items-center justify-center py-40 text-center">
              <p className="text-white text-[11px] font-black uppercase tracking-[0.6em] mb-4">Coming Soon</p>
              <p className="text-neutral-600 text-sm italic font-serif max-w-xs leading-relaxed">Merch drops are in the works. Check back soon or follow on Linktree for updates.</p>
            </div>
